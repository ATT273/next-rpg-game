  ...
  return (
    <div className="w-full flex flex-col gap-4 justify-center items-center">
      <div className="flex gap-2 items-center">
        <div className={cn("relative size-20 border-2 border-zinc-300", isFirstSlotOccupied && "border-amber-300")}>
          {isFirstSlotOccupied && (
            <span
              role="button"
              className="absolute top-0 right-0 p-0.5 translate-x-1/2 -translate-y-1/2 cursor-pointer rounded-full bg-white border border-red-500 text-red-500"
        ...
        >
          <Anvil className="size-6 text-zinc-500" />
        </HoldButton>
        <div className={cn("relative size-20 border-2 border-zinc-300", isSecondSlotOccupied && "border-amber-300")}>
      ...
      {!canAffordForge && (
        <div className="w-full">
          <p className="text-red-500 text-sm">You don't have enough gold to forge item</p>
        </div>
      )}
          ...
          player.items.map((item) => {
            return (
              <div key={item.instanceId} onClick={() => handleSelectItemToForge(item)} className="cursor-pointer">
                <Image src={item.image} alt={`${item.name}-icon`} width={80} height={80} className="" />
              </div>
            );
          })
        ) : (
          <p className="text-zinc-500">No items to forge</p>
        )}
      </div>
    </div>
  );
